...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def loss(self,
        ...
        target_maps_list, neg_maps_list, anchor_bboxes = self.get_targets(
            anchor_list, responsible_flag_list, gt_bboxes, gt_labels)

        losses_cls, losses_conf, losses_xy, losses_wh, losses_bbox = multi_apply(
            self.loss_single, pred_maps, target_maps_list, neg_maps_list, anchor_bboxes, gt_bboxes)
    ...
    def loss_single(self, pred_map, target_map, neg_map, anchor_bboxes, gt_bboxes):
        ...
        loss_bbox = self.loss_bbox(anchor_bboxes.unsqueeze(0), gt_bboxes.unsqueeze(0))

        return loss_cls, loss_conf, loss_xy, loss_wh, loss_bbox
    ...
    def get_targets(self, anchor_list, responsible_flag_list, gt_bboxes_list,
        ...
        all_target_maps, all_neg_maps, anchor_bboxes = results
        assert num_imgs == len(all_target_maps) == len(all_neg_maps)
        target_maps_list = images_to_levels(all_target_maps, num_level_anchors)
        neg_maps_list = images_to_levels(all_neg_maps, num_level_anchors)

        return target_maps_list, neg_maps_list, anchor_bboxes
    ...
    def _get_targets_single(self, anchors, responsible_flags, gt_bboxes,
        ...
        # anchor_bboxes = torch.cat((sampling_result.pos_bboxes, sampling_result.neg_bboxes), axis=0)
        anchor_bboxes = sampling_result.pos_bboxes
        return target_map, neg_map, anchor_bboxes
